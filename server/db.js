...
export async function initDb() {
  ...
  const studentCountRow = await db.get('SELECT COUNT(*) as count FROM students')
  if (!studentCountRow || studentCountRow.count === 0) {
    for (const seeded of seededStudents) {
      ...
      const predictedScore = calculatePredictedScore(seeded.attendance, seeded.marks, seeded.interactionScore)
